...
static void send_resc_used(int cmd, int count, ruu *rud);
static void send_pending_ruu();
static void send_pending_updates(int force);
...
ruu *
get_job_update(job *pjob, int use_rtn_list_ext)
{
	/*
	...
	for (i = 0; mom_rtn_list[i] != JOB_ATR_LAST; ++i) {
		...
		if ((at->at_flags & ATR_VFLAG_MODIFY) || (svr_hook_resend_job_attrs != 0 && (at->at_flags & ATR_VFLAG_HOOK))) {
	...
	if (use_rtn_list_ext) {
		for (i = 0; mom_rtn_list_ext[i] != JOB_ATR_LAST; ++i) {
			nth = mom_rtn_list_ext[i];
			at = &pjob->ji_wattr[nth];
			ad = &job_attr_def[nth];
...
int
enqueue_update_for_send(job *pjob, int cmd, int use_rtn_list_ext)
{
	ruu *prused = get_job_update(pjob, use_rtn_list_ext);
	if (prused == NULL)
		return 1; /* get_job_update has done error logging */

	if (mom_pid != getpid()) {
		/* If child, send update right a way */
