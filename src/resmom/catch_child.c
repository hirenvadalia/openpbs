...
{
	enqueue_update_for_send(pjob, cmd, use_rtn_list_ext);
}
...
update_jobs_status(void)
{
	...
	for (; pjob; pjob = (job *)GET_NEXT(pjob->ji_alljobs)) {
		...
		if (pjob->ji_qs.ji_substate != JOB_SUBSTATE_RUNNING)
			continue;
		enqueue_update_for_send(pjob, IS_RESCUSED, 0);
	}
	svr_hook_resend_job_attrs = 0;
}
...
send_obit(job *pjob, int exval)
{
	...
	if (enqueue_update_for_send(pjob, IS_JOBOBIT, 0) != 0) {
