...
static int
router_send_ctl_join(int tfd, void *data, void *c, void *extra)
{
...
router_post_connect_handler(int tfd, void *data, void *c, void *extra)
{
	...
	return router_send_ctl_join(tfd, data, c, extra);
}
...
router_pkt_handler(int tfd, void *data, int len, void *c, void *extra)
{
	...
	if (type == TPP_AUTH_CTX) {
		...
		if (extra == NULL && tpp_conf->alloc_auth_extra != NULL) {
			extra = tpp_conf->alloc_auth_extra(TPP_AUTH_SERVER);
			if (extra == NULL) {
				free(data_in);
				tpp_log_func(LOG_CRIT, __func__, "Failed to alloc server auth extra");
				return -1;
			}
			tpp_transport_set_conn_extra(tfd, extra);
		}
