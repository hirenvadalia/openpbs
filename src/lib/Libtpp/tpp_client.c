 ...
 *		destinations. LEAF (end) nodes are connected to router via
...
leaf_send_ctl_join(int tfd, void *c)
{
	...
	if (ctx->type == TPP_ROUTER_NODE) {
		...
		if (tpp_transport_vsend(router.conn_fd, pkt) != 0) { /* this has to go irrespective of router state being down */
...
tpp_init(struct tpp_config *cnf)
{
	...
	/* initialize the router structure and initiate connection to it */
	router.router_name = tpp_conf->routers[0];
	router.conn_fd = -1;
	router.initiator = 1;
	router.state = TPP_ROUTER_STATE_DISCONNECTED;
	router.index = 0;
	router.delay = 0;

	tpp_log(LOG_INFO, NULL, "Connecting to pbs_comm %s", router.router_name);

	/* connect to router and send initial join packet */
	if ((rc = connect_router(&router)) != 0)
		return -1;
	...
	if (pthread_atfork(tpp_client_prefork, tpp_client_postfork_parent, tpp_client_postfork_child)) {
		tpp_log(LOG_CRIT, __func__, "TPP atfork handler registration failed");
		return -1;
	}
#endif
...
queue_strm_close(stream_t *strm)
{
	...
	tpp_read_lock(&strmarray_lock); /* already under lock, dont need get_strm_atomic */
	...
	tpp_transport_wakeup_thrd(router.conn_fd);
	return;
}
...
leaf_pkt_presend_handler(int tfd, tpp_packet_t *pkt, void *extra)
{
	...
	if (type == TPP_CTL_JOIN) {
		router.state = TPP_ROUTER_STATE_CONNECTED;
		router.delay = 0; /* reset connection retry time to 0 */
		router.conn_time = time(0); /* record connect time */

		tpp_log(LOG_CRIT, __func__, "Connected to pbs_comm %s", router.router_name);
...
{
	if ((router.conn_fd == -1) || (router.state != TPP_ROUTER_STATE_CONNECTED)) {
