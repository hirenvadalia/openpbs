 ...
 */
int
wire_decode_queuejob(void *buf, breq *request)
{
	ns(Qjob_table_t) B = (ns(Qjob_table_t)) ns(Req_body((ns(Req_table_t))buf));
	...
	return wire_decode_svrattrl((void *)ns(Qjob_attrs(B)), &(request->rq_ind.rq_queuejob.rq_attr));
}
