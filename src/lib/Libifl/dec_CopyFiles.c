 ...
 */
static int
__wire_decode_batch_req_copyfiles_helper(ns(CopyFile_table_t) B, struct rq_cpyfile *pcf)
{
	int pair_ct = 0;
 ...
 */
int
wire_decode_batch_req_copyfiles(void *buf, breq *request)
{
	ns(CopyHook_table_t) B = (ns(CopyHook_table_t))buf;

	return __wire_decode_batch_req_copyfiles_helper(B, &(request->rq_ind.rq_cpyfile));
}
 ...
 */
int
wire_decode_batch_req_copyfiles_cred(void *buf, breq *request)
{
	...
	return __wire_decode_batch_req_copyfiles_helper(ns(CopyFileCred_files(B)), &(request->rq_ind.rq_cpyfile_cred.rq_copyfile));
}
