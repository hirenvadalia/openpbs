 ...
 */
void
wire_chan_set_ctx_status(int fd, int status, int for_encrypt)
{
	pbs_tcp_chan_t *chan = wire_get_chan(fd);
	if (chan == NULL)
		return;
...
/**
 * @brief
 * 	wire_chan_get_ctx_status - get auth context status tcp chan assosiated with given fd
 *
 * @param[in] fd - file descriptor
 ...
 */
int
wire_chan_get_ctx_status(int fd, int for_encrypt)
{
	pbs_tcp_chan_t *chan = wire_get_chan(fd);
	if (chan == NULL)
		return -1;
...
/**
 * @brief
 * 	wire_chan_set_authctx - associates authenticaion context with connection
 *
 * @param[in] fd - file descriptor
 ...
 */
void
wire_chan_set_authctx(int fd, void *authctx, int for_encrypt)
{
	pbs_tcp_chan_t *chan = wire_get_chan(fd);
	if (chan == NULL)
		return;
...
/**
 * @brief
 * 	wire_chan_get_authctx - gets authentication context associated with connection
 *
 * @param[in] fd - file descriptor
 ...
 */
void *
wire_chan_get_authctx(int fd, int for_encrypt)
{
	pbs_tcp_chan_t *chan = wire_get_chan(fd);
	if (chan == NULL)
		return NULL;
...
/**
 * @brief
 * 	wire_chan_set_authdef - associates authdef structure with connection
 *
 * @param[in] fd - file descriptor
 ...
 */
void
wire_chan_set_authdef(int fd, auth_def_t *authdef, int for_encrypt)
{
	pbs_tcp_chan_t *chan = wire_get_chan(fd);
	if (chan == NULL)
		return;
...
/**
 * @brief
 * 	wire_chan_get_authdef - gets authdef structure associated with connection
 *
 * @param[in] fd - file descriptor
 ...
 */
auth_def_t *
wire_chan_get_authdef(int fd, int for_encrypt)
{
	pbs_tcp_chan_t *chan = wire_get_chan(fd);
	if (chan == NULL)
		return NULL;
...
/**
 * @brief
 * 	wire_chan_is_encrypted - is chan assosiated with given fd is encrypted?
 *
 * @param[in] fd - file descriptor
 *
 * @return int
...
int
wire_send_pkt(int fd, int type, void *data_in, size_t len_in)
{
	int i = 0;
	int ndlen = 0;
	void *pkt = NULL;
...
int
wire_recv_pkt(int fd, int *type, void **data_out, size_t *len_out)
{
	int i = 0;
	...
	*len_out = 0;
	...
	i = wire_recv(fd, (void *)type, 1);
	if (i != 1)
		return i;

	i = wire_recv(fd, (void *)&ndbuf, sizeof(int));
	if (i != sizeof(int))
		return i;
...
{
	void *authctx = wire_chan_get_authctx(fd, FOR_ENCRYPT);
	auth_def_t *authdef = wire_chan_get_authdef(fd, FOR_ENCRYPT);
	void *data_out = NULL;
	size_t len_out = 0;
	int rc = 0;
	...
	rc = wire_send_pkt(fd, AUTH_ENCRYPTED_DATA, data_out, len_out);
	if (rc <= 0) {
		free(data_out);
		return rc;
	}
...
dis_recv_and_decrypt(int fd, void **data_out, size_t *len_out)
{
	...
	*len_out = 0;
	...
	rc = wire_recv_pkt(fd, &type, &data_in, &len_in);
	if (rc <= 0)
		return rc;
...
/**
 * @brief
 * 	__wire_read - read data from connection to "fill" the buffer
...
__wire_read(int fd)
{
	...
	if (tp == NULL)
		return -1;
	dis_pack_buf(tp);
	if (wire_chan_is_encrypted(fd)) {
		i = dis_recv_and_decrypt(fd, &data, &len);
	} else {
		int type; /* unused */
		i = wire_recv_pkt(fd, &type, &data, &len);
	}
	if (i <= 0)
		return i;
...
dis_flush(int fd)
{
	...
	if (wire_chan_is_encrypted(fd)) {
		...
		/* DIS doesn't have pkt type, pass 0 always for type in wire_send_pkt */
		if (wire_send_pkt(fd, 0, (void *)tp->tdis_thebuf, tp->tdis_trail) <= 0)
			return -1;
	}
...
dis_destroy_chan(int fd)
{
	...
	if (pfn_wire_get_chan == NULL)
		return;
	chan = wire_get_chan(fd);
	if (chan != NULL) {
 ...
 */
void
dis_setup_chan(int fd, pbs_tcp_chan_t * (*inner_wire_get_chan)(int))
{
	pbs_tcp_chan_t *chan;
	int rc;
	...
	if (fd < 0)
		return;
	chan = (pbs_tcp_chan_t *)(*inner_wire_get_chan)(fd);
	if (chan == NULL) {
		if (errno == ENOTCONN)
			return;
