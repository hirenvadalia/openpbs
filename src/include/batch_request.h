...
struct rq_auth_ext {
	...
	union {
		struct
		{
			char rq_authkey[PBS_AUTH_KEY_LEN + 1];
		} rq_munge;
...
struct batch_request {
	...
	union indep_request {
		...
		struct rq_auth_ext		rq_auth_ext; // FIXME: merge all auth struct
		int				rq_connect;
		struct rq_queuejob		rq_queuejob;
		struct rq_jobcred		rq_jobcred; // FIXME: merge all cred struct
...
extern int encode_wire_CopyFiles_Cred(int, struct batch_request *);
extern int encode_wire_Register(int, struct batch_request *);
extern int encode_wire_TrackJob(int, struct batch_request *);
extern int encode_wire_reply(int, struct batch_reply *);
extern int encode_wire_replyTPP(int, char *, struct batch_reply *);
extern int encode_wire_svrattrl(int, svrattrl *);
